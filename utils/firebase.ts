...
export const getSchool = async (schoolId: string) => {
  try {
    ...
    if (schoolDoc.exists()) {
      ...
      console.log(`School data retrieved: ${JSON.stringify(data, (key, value) => 
        // Don't log the entire location object which might be circular
        key === 'location' ? '[GeoPoint]' : value
      )}`);
      
      // Make sure to properly handle GeoPoint data
      if (data.location && typeof data.location === 'object') {
        // GeoPoint objects have latitude and longitude methods
        if ('latitude' in data.location && 'longitude' in data.location) {
          // Return structured location data for easier access
          return { 
            id: schoolDoc.id, 
            ...data,
            location: {
      ...
      return { id: schoolDoc.id, ...data };
    } else {
      console.error(`School with ID ${schoolId} not found`);
      throw new Error('School not found');
    }
  } catch (error: any) {
    console.error(`Error getting school: ${error.message}`, error);
...
export const loadSchoolDataFromRef = async (schoolRef: any) => {
  try {
    ...
    // Get routes and exclude soft-deleted ones
    const routesRef = collection(db, schoolRef.path, 'Routes');
    const routesQuery = query(routesRef, where('soft_delete', '!=', true));
    ...
    const routesWithStops = await Promise.all(
      routes.map(async (route) => {
        ...
        const stopsQuery = query(stopsRef, where('soft_delete', '!=', true));
...
// Add a function to directly get the current user's school ID
export const getCurrentUserSchoolId = async (): Promise<string | null> => {
  try {
    ...
    const cachedSchoolId = await AsyncStorage.getItem('currentSchoolId');
    if (cachedSchoolId) {
      console.log(`Retrieved school ID from cache: ${cachedSchoolId}`);
      return cachedSchoolId;
    }
    
    // Otherwise, look it up in Manager-School collection
    console.log(`Looking up school for manager: ${currentUser.uid}`);
    const managerSchoolRef = doc(db, 'Manager-School', currentUser.uid);
    const managerSchoolDoc = await getDoc(managerSchoolRef);
    
    if (!managerSchoolDoc.exists()) {
      console.error(`No school mapping found for manager ${currentUser.uid}`);
      return null;
    }
    ...
    const schoolRef = managerSchoolDoc.data()['school-collection'];
    if (!schoolRef) {
      console.error('No school-collection reference found in Manager-School document');
      return null;
    }
