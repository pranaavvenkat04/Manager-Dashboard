...
const fetchRoutes = async (): Promise<Route[]> => {
  try {
    ...
    // Only get routes that are not soft deleted or where soft_delete is not set
    const q = query(routesRef, where('soft_delete', '!=', true));
    const snapshot = await getDocs(q);
    ...
    return snapshot.docs.map(doc => ({
      ...
      soft_delete: doc.data().soft_delete || false
    }));
  } catch (error) {
